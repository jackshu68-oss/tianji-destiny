...
(function () {
  ...
  function initMeihua() {
    ...
    $('#mh-result').addEventListener('click', event => {
      if (event.target.closest('[data-mh-detail]') && lastMeihua) detail('梅花易数 · 详细报告', buildMeihuaDetail(lastMeihua));
    });
  }
  function runMeihua() {
    try {
  ...
  function renderMeihua(result) {
    ...
    $('#mh-result').innerHTML = `
      ...
      <button type="button" class="detail-action" data-mh-detail>查看详细报告</button>
      ${source('meihua')}`;
  ...
  function initQimen() {
    ...
    $('#qm-result').addEventListener('click', event => {
      ...
      if (palace && lastQimen) detail(`${palace.dataset.qmGong}宫 · 奇门报告`, buildQimenPalaceDetail(lastQimen, palace.dataset.qmGong));
      if (event.target.closest('[data-qm-detail]') && lastQimen) detail('奇门遁甲 · 详细报告', buildQimenDetail(lastQimen, lastQimenMeta));
    });
  }
  function runQimen() {
  ...
  function renderQimen(pan, meta) {
    ...
    $('#qm-result').innerHTML = `
      ...
      <button type="button" class="detail-action" data-qm-detail>查看详细报告</button>
      ${source('qimen')}`;
