var Ziwei = (() => {
  ...
  // node_modules/ziwei/dist/index.js
  var index_exports = {};
  ...
  var AbstractCulture = class {
    ...
    equals(o2) {
      return o2 && o2.toString() === this.toString();
    }
    indexOf(index, size) {
      let i = index % size;
      if (i < 0) {
  ...
  var WeekUnit = class _WeekUnit extends MonthUnit {
    ...
    static validate(_year, _month, index, start) {
      _WeekUnit.validateRange(index, 0, 5, "week index");
      _WeekUnit.validateRange(start, 0, 6, "week start");
    }
  };
  var SecondUnit = class _SecondUnit extends DayUnit {
    hour;
    minute;
    second;
    ...
    static validate(_year, _month, _day, hour, minute, second) {
      _SecondUnit.validateRange(hour, 0, 23, "hour");
      _SecondUnit.validateRange(minute, 0, 59, "minute");
      _SecondUnit.validateRange(second, 0, 59, "second");
    }
  };
  ...
  var Element = class _Element extends LoopTyme {
    ...
    constructor(names, indexOrName) {
      super(names === null ? _Element.NAMES : names, indexOrName);
    }
    static fromIndex(index) {
      return new _Element(null, _Element.numeric(index, "element index"));
    }
    static fromName(name) {
      return new _Element(null, name);
    }
    next(n) {
  ...
  var EarthBranch = class _EarthBranch extends LoopTyme {
    ...
    getHideHeavenStems() {
      ...
      let o2 = this.getHideHeavenStemMiddle();
      if (o2) {
        l.push(new HideHeavenStem(
          o2,
          1
      ...
      o2 = this.getHideHeavenStemResidual();
      if (o2) {
        l.push(new HideHeavenStem(
          o2,
          0
  ...
  var Phenology = class _Phenology extends LoopTyme {
    ...
    constructor(year, indexOrName) {
      ...
      const y = _Phenology.numeric(year, "phenology year");
      if (typeof indexOrName === "number") {
        const size = this.getSize();
  ...
  var NineStar = class _NineStar extends LoopTyme {
    ...
    getColor() {
      return _NineStar.COLORS[this.index];
    }
    getElement() {
  ...
  var LunarYear = class _LunarYear extends YearUnit {
    ...
    static init() {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        for (let y = 0; y < size; y++) {
          ...
          let c2 = 1;
          for (let x = 1; x > -1; x--) {
            t += c2 * chars.indexOf(m.charAt(z + x));
            c2 *= 64;
          }
          n += t;
          l.push(n);
    ...
    static validate(year) {
      _LunarYear.validateRange(year, -1, 9999, "lunar year");
    }
  ...
  var LunarMonth = class _LunarMonth extends MonthUnit {
    ...
    next(n) {
      if (n === 0) {
        return _LunarMonth.fromYm(this.year, this.getMonthWithLeap());
      }
  ...
  var LunarWeek = class _LunarWeek extends WeekUnit {
    ...
    next(n) {
      ...
      return _LunarWeek.fromYm(m.getYear(), m.getMonthWithLeap(), d, this.start);
    }
    getFirstDay() {
    ...
    equals(o2) {
      return o2 && o2.getFirstDay().equals(this.getFirstDay());
    }
  };
  ...
  var SixtyCycleYear = class _SixtyCycleYear extends AbstractTyme {
    ...
    constructor(year) {
      ...
      _SixtyCycleYear.validateRange(y, -1, 9999, "sixty cycle year");
      this.year = y;
    }
    static fromYear(year) {
  ...
  var LunarHour = class _LunarHour extends SecondUnit {
    ...
    next(n) {
      if (n === 0) {
        return _LunarHour.fromYmdHms(this.year, this.month, this.day, this.hour, this.minute, this.second);
      }
      const h = this.hour + n * 2;
      const d = this.getLunarDay().next(Math.floor(h / 24));
      return _LunarHour.fromYmdHms(d.getYear(), d.getMonth(), d.getDay(), this.indexOf(h, 24), this.minute, this.second);
    }
    isBefore(target) {
      return this.getCompareIndex() < target.getCompareIndex();
    }
  ...
  var JulianDay = class _JulianDay extends AbstractTyme {
    ...
    getSolarTime() {
      let d = ~~(this.day + 0.5);
      let f2 = this.day + 0.5 - d;
      if (d >= 2299161) {
        const c2 = ~~((d - 186721625e-2) / 36524.25);
        d += 1 + c2 - ~~(c2 * 0.25);
      }
      d += 1524;
  ...
  var ShouXingUtil = class _ShouXingUtil {
    ...
    static eLon(t, n) {
      ...
      for (let i = 0; i < 6; i++, tn *= t) {
        ...
        for (let j = n1; j < m; j += 3) {
          c2 += _ShouXingUtil.XL0[j] * Math.cos(_ShouXingUtil.XL0[j + 1] + t * _ShouXingUtil.XL0[j + 2]);
        }
        v += c2 * tn;
      }
    ...
    static mLon(t, n) {
      ...
      for (let i = 0, x = _ShouXingUtil.XL1.length; i < x; i++, tn *= t) {
        ...
        for (j = 0, c2 = 0; j < m; j += 6) {
          c2 += f2[j] * Math.cos(f2[j + 1] + t * f2[j + 2] + t2 * f2[j + 3] + t3 * f2[j + 4] + t4 * f2[j + 5]);
        }
        v += c2 * tn;
      }
    ...
    static ev(t) {
      const f2 = 628.307585 * t;
      return 628.332 + 21 * Math.sin(1.527 + f2) + 0.44 * Math.sin(1.48 + f2 * 2) + 0.129 * Math.sin(5.82 + f2) * t + 55e-5 * Math.sin(4.21 + f2) * t * t;
    }
    static saLon(t, n) {
    ...
    static qiShuo(isQi, isHigh, jd, pc) {
      const w = isQi ? Math.floor((jd + pc - 2451259) / 365.2422 * 24) * Math.PI / 12 : Math.floor((jd + pc - 2451551) / 29.5306) * _ShouXingUtil.PI_2;
      const d = isQi ? isHigh ? _ShouXingUtil.qiHigh(w) : _ShouXingUtil.qiLow(w) : isHigh ? _ShouXingUtil.shuoHigh(w) : _ShouXingUtil.shuoLow(w);
    ...
    static calc(isQi, jd, kb, pc, fkb) {
      ...
      if (j < f1 || j >= 2436935) {
        d = _ShouXingUtil.qiShuo(isQi, true, j, pc);
      } else if (j >= f1 && j < f2) {
        let i;
        for (i = 0; i < size; i += 2) {
          if (j + pc < kb[i + 2]) {
        ...
        d = _ShouXingUtil.qiShuo(isQi, false, j, pc);
    ...
    static calcShuo(jd) {
      return _ShouXingUtil.calc(false, jd, _ShouXingUtil.SHUO_KB, 14, _ShouXingUtil.SB);
    }
    static calcQi(jd) {
      return _ShouXingUtil.calc(true, jd, _ShouXingUtil.QI_KB, 7, _ShouXingUtil.QB);
    }
    static qiAccurate(w) {
  ...
  var SolarTerm = class _SolarTerm extends LoopTyme {
    ...
    static fromIndex(year, index) {
      return new _SolarTerm(year, _SolarTerm.numeric(index, "solar term index"));
    }
    static fromName(year, name) {
  ...
  var SolarYear = class _SolarYear extends YearUnit {
    ...
    static validate(year) {
      _SolarYear.validateRange(year, 1, 9999, "solar year");
    }
    static fromYear(year) {
  ...
  var SolarHalfYear = class _SolarHalfYear extends YearUnit {
    ...
    static validate(year, index) {
      _SolarHalfYear.validateRange(index, 0, 1, "solar half year index");
  ...
  var SolarSeason = class _SolarSeason extends YearUnit {
    ...
    static validate(year, index) {
      _SolarSeason.validateRange(index, 0, 3, "solar season index");
      SolarYear.validate(year);
    }
    static fromIndex(year, index) {
  ...
  var SolarMonth = class _SolarMonth extends MonthUnit {
    ...
    constructor(year, month) {
      const y = _SolarMonth.numeric(year, "solar year");
      const m = _SolarMonth.numeric(month, "solar month");
      _SolarMonth.validate(y, m);
      super(y, m);
    }
    static validate(year, month) {
      _SolarMonth.validateRange(month, 1, 12, "solar month");
      SolarYear.validate(year);
    }
    static fromYm(year, month) {
  ...
  var SolarWeek = class _SolarWeek extends WeekUnit {
    ...
    getIndexInYear() {
      ...
      let w = _SolarWeek.fromYm(this.year, 1, 0, this.start);
    ...
    next(n) {
      ...
      return _SolarWeek.fromYm(m.getYear(), m.getMonth(), d, this.start);
    }
    getFirstDay() {
  ...
  var SolarDay = class _SolarDay extends DayUnit {
    ...
    getSolarWeek(start) {
      return SolarWeek.fromYm(this.year, this.month, Math.ceil((this.getDay() + _SolarDay.fromYmd(this.year, this.month, 1).getWeek().next(-start).getIndex()) / 7) - 1, start);
    }
    getPhenologyDay() {
    ...
    getIndexInYear() {
      return this.subtract(_SolarDay.fromYmd(this.year, 1, 1));
    }
    subtract(target) {
    ...
    getHijriDay() {
      let d = this.subtract(new _SolarDay(622, 7, 16));
  ...
  var SolarTime = class _SolarTime extends SecondUnit {
    ...
    next(n) {
      if (n === 0) {
        return _SolarTime.fromYmdHms(this.year, this.month, this.day, this.hour, this.minute, this.second);
      }
      const t = this.getSecondsInDay() + n;
      const s = this.indexOf(t, 86400);
      const d = this.getSolarDay().next(Math.floor(t / 86400));
      return _SolarTime.fromYmdHms(d.getYear(), d.getMonth(), d.getDay(), ~~(s / 3600), ~~(s % 3600 / 60), s % 60);
    }
    isBefore(target) {
      return this.getCompareIndex() < target.getCompareIndex();
    }
  ...
  var AbstractFestival = class extends AbstractTyme {
    index;
    day;
    event;
  ...
  var SolarFestival = class _SolarFestival extends AbstractFestival {
    ...
    static fromIndex(year, index) {
      const y = _SolarFestival.numeric(year, "solar festival year");
      const i = _SolarFestival.numeric(index, "solar festival index");
      if (i < 0 || i >= _SolarFestival.NAMES.length) {
        return null;
      }
      const start = i * 8;
      const e = new Event(_SolarFestival.NAMES[i], "@" + _SolarFestival.DATA.substring(start, start + 8));
      if (y < e.getStartYear()) {
        return null;
      }
      return new _SolarFestival(i, e, SolarDay.fromYmd(y, e.getValue(2), e.getValue(3)));
    }
    static fromYmd(year, month, day) {
      const d = SolarDay.fromYmd(year, month, day);
      for (let i = 0, j = _SolarFestival.NAMES.length; i < j; i++) {
        const start = i * 8;
        const e = new Event(_SolarFestival.NAMES[i], "@" + _SolarFestival.DATA.substring(start, start + 8));
        if (d.getYear() >= e.getStartYear() && d.getMonth() === e.getValue(2) && d.getDay() === e.getValue(3)) {
          return new _SolarFestival(i, e, d);
        }
      }
      return null;
    }
  ...
  var LunarFestival = class _LunarFestival extends AbstractFestival {
    ...
    static fromIndex(year, index) {
      const y = _LunarFestival.numeric(year, "lunar festival year");
      const i = _LunarFestival.numeric(index, "lunar festival index");
      if (i < 0 || i >= _LunarFestival.NAMES.length) {
        return null;
      }
      const start = i * 8;
      const e = new Event(_LunarFestival.NAMES[i], "@" + _LunarFestival.DATA.substring(start, start + 8));
      switch (e.getType()) {
        case 2:
          const m = e.getMonth(y);
          const d = LunarDay.fromYmd(m[0], m[1], e.getValue(3));
          const offset = e.getValue(5);
          return new _LunarFestival(i, e, offset === 0 ? d : d.next(offset));
        case 3:
          return new _LunarFestival(i, e, SolarTerm.fromIndex(y, e.getValue(2)).getSolarDay().getLunarDay());
      }
      return null;
    }
    static fromYmd(year, month, day) {
      const d = LunarDay.fromYmd(year, month, day);
      for (let i = 0, j = _LunarFestival.NAMES.length; i < j; i++) {
        const start = i * 8;
        const e = new Event(_LunarFestival.NAMES[i], "@" + _LunarFestival.DATA.substring(start, start + 8));
        switch (e.getType()) {
          case 2:
            ...
            if (offset === 0) {
              if (d.getMonth() === e.getValue(2) && d.getDay() === e.getValue(3)) {
                return new _LunarFestival(i, e, d);
              }
            } else {
  ...
  var RabByungElement = class _RabByungElement extends Element {
    ...
    constructor(indexOrName) {
      super(_RabByungElement.NAMES, indexOrName);
    }
    static fromIndex(index) {
  ...
  var RabByungYear = class _RabByungYear extends AbstractTyme {
    ...
    static validate(year) {
      _RabByungYear.validateRange(year, 1027, 9999, "rab-byung year");
    }
    static fromYear(year) {
  ...
  var HijriYear = class _HijriYear extends YearUnit {
    constructor(year) {
      const y = _HijriYear.numeric(year, "hijri year");
      _HijriYear.validate(y);
      super(y);
    }
    static validate(year) {
      _HijriYear.validateRange(year, -640, 9666, "hijri year");
    }
    static fromYear(year) {
      return new _HijriYear(year);
    }
    getDayCount() {
    ...
    next(n) {
      return _HijriYear.fromYear(this.year + n);
    }
    getMonths() {
      const months = [];
      for (let i = 1; i <= 12; i++) {
  ...
  var HijriMonth = class _HijriMonth extends MonthUnit {
    static NAMES = ["\u7A46\u54C8\u5170\u59C6\u6708", "\u8272\u6CD5\u5C14\u6708", "\u8D56\u6BD4\u5C14\xB7\u6556\u5916\u9C81\u6708", "\u8D56\u6BD4\u5C14\xB7\u963F\u8272\u5C3C\u6708", "\u4E3B\u9A6C\u8FBE\xB7\u6556\u5916\u9C81\u6708", "\u4E3B\u9A6C\u8FBE\xB7\u963F\u8272\u5C3C\u6708", "\u8D56\u54F2\u535C\u6708", "\u820D\u5C14\u90A6\u6708", "\u8D56\u4E70\u4E39\u6708", "\u95EA\u74E6\u9C81\u6708", "\u90FD\u5C14\u5580\u5C14\u5FB7\u6708", "\u90FD\u5C14\u9ED1\u54F2\u6708"];
    constructor(year, month) {
      const y = _HijriMonth.numeric(year, "hijri year");
      const m = _HijriMonth.numeric(month, "hijri month");
      _HijriMonth.validate(y, m);
      super(y, m);
    }
    static validate(year, month) {
      _HijriMonth.validateRange(month, 1, 12, "hijri month");
      HijriYear.validate(year);
    }
    static fromYm(year, month) {
      return new _HijriMonth(year, month);
    }
    getHijriYear() {
    ...
    getName() {
      return _HijriMonth.NAMES[this.getIndexInYear()];
    }
    toString() {
    ...
    next(n) {
      ...
      return _HijriMonth.fromYm(~~((this.year * 12 + i) / 12), this.indexOf(i, 12) + 1);
    }
    getDays() {
      const l = [];
  ...
  var HijriDay = class _HijriDay extends DayUnit {
    ...
    constructor(year, month, day) {
      const y = _HijriDay.numeric(year, "hijri year");
      const m = _HijriDay.numeric(month, "hijri month");
      const d = _HijriDay.numeric(day, "hijri day");
      _HijriDay.validate(y, m, d);
      super(y, m, d);
    }
    static validate(year, month, day) {
    ...
    static fromYmd(year, month, day) {
      return new _HijriDay(year, month, day);
    }
    getHijriMonth() {
      return HijriMonth.fromYm(this.year, this.month);
    }
    getName() {
      return _HijriDay.NAMES[this.day - 1];
    }
    toString() {
    ...
    getSolarDay() {
      return SolarDay.fromYmd(622, 7, 16).next(this.subtract(new _HijriDay(1, 1, 1)));
    }
    getIndexInYear() {
      return this.subtract(new _HijriDay(this.year, 1, 1));
    }
  };
  var Event = class _Event extends AbstractCulture {
    ...
    static fromName(name) {
      ...
      return matcher ? new _Event(name, matcher[1]) : null;
    }
    getCharIndex(index) {
  ...
  var EventBuilder = class {
    ...
    content(t, a, b2, c2) {
      ...
      return this.setValue(2, a).setValue(3, b2).setValue(4, c2);
    }
  ...
  var EventManager = class _EventManager {
    ...
    static saveOrUpdate(name, data) {
      const o2 = _EventManager.REGEX + name;
      const matcher = new RegExp(o2, "g").exec(_EventManager.DATA);
      if (matcher) {
        _EventManager.DATA = _EventManager.DATA.replace(o2, "");
      } else {
  ...
  // node_modules/ziwei/dist/index.js
  var c = ["\u5B50", "\u4E11", "\u5BC5", "\u536F", "\u8FB0", "\u5DF3", "\u5348", "\u672A", "\u7533", "\u9149", "\u620C", "\u4EA5"];
  var o = ["\u7532", "\u4E59", "\u4E19", "\u4E01", "\u620A", "\u5DF1", "\u5E9A", "\u8F9B", "\u58EC", "\u7678"];
  var f = ["\u547D\u5BAB", "\u5144\u5F1F", "\u592B\u59BB", "\u5B50\u5973", "\u8D22\u5E1B", "\u75BE\u5384", "\u8FC1\u79FB", "\u4EA4\u53CB", "\u5B98\u7984", "\u7530\u5B85", "\u798F\u5FB7", "\u7236\u6BCD"];
  var g = ((e) => (e[e.Water = 2] = "Water", e[e.Wood = 3] = "Wood", e[e.Gold = 4] = "Gold", e[e.Soil = 5] = "Soil", e[e.Fire = 6] = "Fire", e))(g || {});
  var u = class {
    constructor(s) {
      this._palaces = [];
      this.yinYang = false;
      this.fateType = 5;
      this.birthDay = { year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0, sex: 0 };
      let a = SolarTime.fromYmdHms(s.year, s.month, s.day, s.hour, s.minute, s.second).getLunarHour(), i = a.getEightChar();
      this.eightChar = i, this.birthDay = { year: a.getYear(), month: a.getMonth(), day: a.getDay(), hour: a.getHour(), minute: a.getMinute(), second: a.getSecond(), sex: s.sex };
      let e = i.getYear().getHeavenStem().toString(), n = i.getMonth().getEarthBranch().toString(), m = i.getHour().getEarthBranch().toString();
      c.forEach((h, x) => {
        this._palaces.push({ stemBranch: { stem: "", branch: h }, stars: [] });
      });
      let p = o.indexOf(M(e));
      for (let h = 0; h < 12; h++) this._palaces[(h + 2) % 12].stemBranch.stem = o[p % 10], o[p % 10] === e && (this._palaces[(h + 2) % 12].isOrigin = true), p++;
      this.yinYang = o.indexOf(e) % 2 === 0;
      let r = Math.abs(c.indexOf(n) - c.indexOf(m)) % 12, d = (c.indexOf(n) + c.indexOf(m)) % 12;
      this._palaces[r].isMain = true, this._palaces[d].isBody = true;
      for (let h = 12; h >= 1; h--) this._palaces[(h + r) % 12].duty = f[12 - h];
      this.fateType = b(this._palaces[r].stemBranch), this.setZiweiStars(), this.setTianfuStars(), this.setNianStars(), this.setMoonStars(), this.setHourStars();
    }
    getPalaces() {
    ...
    setZiweiStars() {
      let s = 0;
      if (this.birthDay.day != 0 && this.birthDay.day % this.fateType === 0) s = this.birthDay.day / this.fateType;
      else if (this.birthDay.day != 0 && this.birthDay.day % this.fateType !== 0) {
        let e = (Math.floor(this.birthDay.day / this.fateType) + 1) * this.fateType - this.birthDay.day;
        e % 2 === 0 ? s = e + Math.floor(this.birthDay.day / this.fateType) + 1 : s = Math.floor(this.birthDay.day / this.fateType) + 1 - e;
      }
      let a = { "-1": 0, 11: 0, 0: 1, 12: 1, 1: 2, 13: 2, 2: 3, 14: 3, 3: 4, 15: 4, 4: 5, "-8": 5, 5: 6, "-7": 6, 6: 7, "-6": 7, 7: 8, "-5": 8, 8: 9, "-4": 9, 9: 10, "-3": 10, 10: 11, "-2": 11 }[s.toString()], i = [{ name: "\u7D2B\u5FAE", type: 0 }, { name: "\u5929\u673A", type: 0 }, , { name: "\u592A\u9633", type: 0 }, { name: "\u6B66\u66F2", type: 0 }, { name: "\u5929\u540C", type: 0 }, , , { name: "\u5EC9\u8D1E", type: 0 }];
      for (let e of i) e && this._palaces[a].stars.push(e), a = (a + 12 - 1) % 12;
    }
    setTianfuStars() {
      let s = this._palaces.findIndex((e) => e.stars.find((n) => n.name === "\u7D2B\u5FAE")), a = { 2: 2, 8: 8, 1: 3, 0: 4, 11: 5, 10: 6, 9: 7, 3: 1, 4: 0, 5: 11, 6: 10, 7: 9 }[s], i = [{ name: "\u5929\u5E9C", type: 0 }, { name: "\u592A\u9634", type: 0 }, { name: "\u8D2A\u72FC", type: 0 }, { name: "\u5DE8\u95E8", type: 0 }, { name: "\u5929\u76F8", type: 0 }, { name: "\u5929\u6881", type: 0 }, { name: "\u4E03\u6740", type: 0 }, , , , { name: "\u7834\u519B", type: 0 }];
      for (let e of i) e && this._palaces[a].stars.push(e), a = (a + 1) % 12;
    }
    setNianStars() {
      let s = this.eightChar.getYear().getHeavenStem().toString(), a = { \u7532: 2, \u620A: 5, \u4E59: 3, \u5DF1: 6, \u4E19: 5, \u5E9A: 8, \u4E01: 6, \u8F9B: 9, \u58EC: 11, \u7678: 0 }[s];
      this._palaces[a].stars.push({ name: "\u7984\u5B58", type: 1 }), a = (a + 12 - 1) % 12, this._palaces[a].stars.push({ name: "\u9640\u7F57", type: 1 }), a = (a + 12 + 2) % 12, this._palaces[a].stars.push({ name: "\u64CE\u7F8A", type: 1 });
      let i = { \u7532: ["\u4E11", "\u672A"], \u620A: ["\u4E11", "\u672A"], \u5E9A: ["\u4E11", "\u672A"], \u4E59: ["\u5B50", "\u7533"], \u5DF1: ["\u5B50", "\u7533"], \u4E19: ["\u4EA5", "\u9149"], \u4E01: ["\u4EA5", "\u9149"], \u8F9B: ["\u5348", "\u5BC5"], \u58EC: ["\u536F", "\u5DF3"], \u7678: ["\u536F", "\u5DF3"] };
      this._palaces.find((n) => n.stemBranch.branch === i[s][0]).stars.push({ name: "\u5929\u9B41", type: 1 }), this._palaces.find((n) => n.stemBranch.branch === i[s][1]).stars.push({ name: "\u5929\u94BA", type: 1 }), a = this._palaces.findIndex((n) => n.stemBranch.branch === "\u536F");
      let e = c.findIndex((n) => n === this.eightChar?.getYear().getEarthBranch().toString());
      a = (a + 12 - e) % 12, this._palaces[a].stars.push({ name: "\u7EA2\u9E3E", type: 1 }), a = (a + 12 + 6) % 12, this._palaces[a].stars.push({ name: "\u5929\u559C", type: 1 });
    }
    setMoonStars() {
      let s = this.birthDay.month - 1, t = this._palaces.findIndex((e) => e.stemBranch.branch === "\u8FB0");
      t = (t + s) % 12, this._palaces[t].stars.push({ name: "\u5DE6\u8F85", type: 2 }), t = this._palaces.findIndex((e) => e.stemBranch.branch === "\u620C"), t = (t + 12 - s) % 12, this._palaces[t].stars.push({ name: "\u53F3\u5F3C", type: 2 }), t = this._palaces.findIndex((e) => e.stemBranch.branch === "\u9149"), t = (t + s) % 12, this._palaces[t].stars.push({ name: "\u5929\u5211", type: 2 }), t = this._palaces.findIndex((e) => e.stemBranch.branch === "\u4E11"), t = (t + s) % 12, this._palaces[t].stars.push({ name: "\u5929\u59DA", type: 2 });
      let i = { 2: "\u5DF3", 6: "\u5DF3", 10: "\u5DF3", 1: "\u7533", 5: "\u7533", 9: "\u7533", 3: "\u5BC5", 7: "\u5BC5", 11: "\u5BC5", 4: "\u4EA5", 8: "\u4EA5", 12: "\u4EA5" }[this.birthDay.month];
      t = this._palaces.findIndex((e) => e.stemBranch.branch === i), this._palaces[t].stars.push({ name: "\u5929\u9A6C", type: 2 });
    }
    setHourStars() {
      let s = c.findIndex((r) => r === this.eightChar?.getHour().getEarthBranch().toString()), t = this._palaces.findIndex((r) => r.stemBranch.branch === "\u620C");
      t = (t + 12 - s) % 12, this._palaces[t].stars.push({ name: "\u6587\u660C", type: 3 }), t = this._palaces.findIndex((r) => r.stemBranch.branch === "\u8FB0"), t = (t + s) % 12, this._palaces[t].stars.push({ name: "\u6587\u66F2", type: 3 });
      let i = { \u5BC5: "\u4E11", \u5348: "\u4E11", \u620C: "\u4E11", \u7533: "\u5BC5", \u5B50: "\u5BC5", \u8FB0: "\u5BC5", \u5DF3: "\u536F", \u9149: "\u536F", \u4E11: "\u536F", \u4EA5: "\u9149", \u672A: "\u9149" }[this.eightChar.getYear().getEarthBranch().toString()];
      t = this._palaces.findIndex((r) => r.stemBranch.branch === i);
      let e = this.eightChar.getHour().getEarthBranch().toString(), n = c.findIndex((r) => r === e);
      t = (t + n) % 12, this._palaces[t].stars.push({ name: "\u706B\u661F", type: 3 });
      let p = { \u5BC5: "\u536F", \u5348: "\u536F", \u620C: "\u536F", \u7533: "\u620C", \u5B50: "\u620C", \u8FB0: "\u620C", \u5DF3: "\u620C", \u9149: "\u620C", \u4E11: "\u620C", \u4EA5: "\u620C", \u672A: "\u620C", \u536F: "\u620C" }[this.eightChar.getYear().getEarthBranch().toString()];
      t = this._palaces.findIndex((r) => r.stemBranch.branch === p), n = c.findIndex((r) => r === e), t = (t + n) % 12, this._palaces[t].stars.push({ name: "\u94C3\u661F", type: 3 });
    }
  };
  function b(l) {
    let s = [4, 2, 6, 5, 3], t = 0;
    for (let a of ["\u7532\u4E59", "\u4E19\u4E01", "\u620A\u5DF1", "\u5E9A\u8F9B", "\u58EC\u7678"]) {
      if (a.includes(l.stem)) {
        let i = [];
        for (let n = 0; n < 3; n++) i.push(s[(t + n) % 5]);
        let e = 0;
        for (let n of ["\u5B50\u4E11", "\u5BC5\u536F", "\u8FB0\u5DF3", "\u5348\u672A", "\u7533\u9149", "\u620C\u4EA5"]) {
          if (n.includes(l.branch)) return i[e % 3];
          e++;
        }
      }
      t++;
  ...
  function M(l) {
    let s = ["\u4E19", "\u620A", "\u5E9A", "\u58EC", "\u7532"];
    for (let t = 0; t < 5; t++) if ([o[t], o[t + 5]].includes(l)) return s[t];
  }
  return __toCommonJS(index_exports);
})();
if (typeof module!=="undefined"){try{module.exports=Ziwei;}catch(e){}}
